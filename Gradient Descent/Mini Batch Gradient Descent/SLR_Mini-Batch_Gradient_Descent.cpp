...
int main()
{
    ...
    while(k<10){
	    ...
	    for(int i=0;i<train_Data_Size;i=i+mini_batch){
            for(int j=i;j<i+mini_batch;++j)
                y_pred[j]=b_0 + (b_1 * X_train[j]);
            double error_gradient_b_0=0, error_gradient_b_1=0;
            for(int j=i;j<i+mini_batch;++j){
                error_gradient_b_0+=(y_pred[j]-Y_train[j]);
                error_gradient_b_1+=(y_pred[j]-Y_train[j])*X_train[j];
            }
            error_gradient_b_0/=mini_batch;
            error_gradient_b_1/=mini_batch;
